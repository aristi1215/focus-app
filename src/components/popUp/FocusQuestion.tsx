  ...
  return (
    <div  className="flex flex-col h-full text-center gap-10">
      ...
      <div className="flex justify-evenly flex-wrap md:flex-nowrap">
        {[...Array(5)].map((_, i) => (
          <button onClick={() => setSession({...session, focus_level: i + 1})} className={`text-black rounded-full border border-[#737373]/50 hover:bg-black hover:text-white transition-all duration-300 cursor-pointer w-10 h-10 p-8 flex items-center justify-center ${session.focus_level === i + 1 ? "bg-black text-white" : ""}`}>
